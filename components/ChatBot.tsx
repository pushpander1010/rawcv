...
export default function ChatBot({ mode = "build", onComplete, onEnd }: Props) {
  const { state, setState, pushUndo, undo, isHydrated, clearChat } = useResume();
  ...
  const sendMessage = useCallback(
    ...
    [messages, loading, mode, sectionHistory, applyResumeUpdate, pushUndo, onComplete]
  );
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      >
        ...
        {loading && (
          <div className="flex justify-start w-full">
            <div className="bg-slate-100 dark:bg-slate-800 rounded-2xl rounded-bl-sm px-4 py-2 w-full max-w-xs">
      ...
      {/* Input area */}
      <div className="border-t border-slate-200 dark:border-slate-700 px-4 py-3 flex flex-col gap-2">
        <div className="flex gap-2 items-end">
          ...
          <button
            ...
            className="shrink-0 px-4 py-2 rounded-xl bg-brand-600 hover:bg-brand-700 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold transition-all shadow-md shadow-brand-500/10 hover:shadow-brand-500/20 focus:outline-none focus:ring-2 focus:ring-brand-500"
          >
            Send
          </button>
        </div>
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            {messages.length > 0 && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-xs text-slate-500 dark:text-slate-300 hover:bg-amber-50 dark:bg-amber-950/30 hover:text-amber-600 hover:border-amber-200 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-400 disabled:opacity-40"
              >
          ...
          {onEnd && (
            <button
              type="button"
              onClick={onEnd}
              className="ml-auto px-4 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-xs text-slate-500 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 dark:bg-slate-800 hover:text-red-500 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-400"
            >
              End Chat
            </button>
          )}
