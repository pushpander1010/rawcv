...
function ImpactBadge({ impact }: { impact: ATSIssue["impact"] }) {
  return (
    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full shrink-0 ${IMPACT_STYLES[impact]}`}>
      {impact}
    </span>
  );
}
...
export default function ATSScoreCard({ result, loading = false }: ATSScoreCardProps) {
  ...
  const label =
    ...
    score >= 60 ? "Moderate ATS compatibility" :
  ...
  return (
    ...
    >
      ...
      {/* Issue list — only shown when score < 60 */}
      {showIssues && (
        <div>
      ...
      {/* All-clear message when score >= 60 */}
      {!showIssues && score >= 60 && (
        <p className="text-sm text-center text-gray-500 dark:text-gray-400">
          Your resume passes basic ATS checks.
          {issues.length > 0 && " Minor improvements may still help."}
        </p>
      )}
    </section>
  );
  ...
  return (
    <li className="rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between gap-3 px-4 py-3 text-left focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500"
