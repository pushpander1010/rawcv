...
const THEMES: ThemeMeta[] = [
  ...
  },
  {
    ...
    accent: "bg-gray-200 dark:bg-slate-700",
    preview: "Light · Airy",
  },
  {
    id: "executive",
    ...
    <div className="w-full">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <input
          ...
          className="flex-1 rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-2.5 text-sm text-gray-800 dark:text-slate-100 placeholder-gray-500 dark:placeholder-slate-400 focus:outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all duration-200 shadow-sm"
        />
            ...
            <button
              ...
              className={`px-3.5 py-2 rounded-xl text-xs font-semibold hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-brand-500/10 ${
                activeCategory === cat
                  ...
                  : "bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-slate-300 border border-gray-200 dark:border-slate-700 hover:bg-gray-100 dark:hover:bg-slate-800"
              }`}
            >
              {cat}
            </button>
      ...
      {/* Theme grid */}
      {filtered.length === 0 ? (
        <p className="text-sm text-gray-600 dark:text-slate-300 text-center py-8">No themes match your search.</p>
      ) : (
        <div
            ...
            return (
              <button
                ...
                className={`group rounded-2xl border-2 overflow-hidden text-left hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-brand-500/10 ${
                  isSelected
                    ? "border-brand-600 shadow-lg shadow-brand-500/10"
                    : "border-gray-200 dark:border-slate-700 hover:border-brand-400 hover:shadow-md"
                }`}
              >
                {/* Thumbnail */}
                <div className={`h-24 ${theme.accent} relative flex items-end p-3`}>
                  ...
                  <div className="w-full space-y-1 opacity-30">
                    <div className="h-2 bg-white dark:bg-slate-900 rounded w-3/4" />
                    <div className="h-1.5 bg-white dark:bg-slate-900 rounded w-1/2" />
                ...
                <div className="p-3 bg-white dark:bg-slate-900">
                  ...
                  <div className="text-xs text-gray-500 dark:text-slate-300">{theme.preview}</div>
                </div>
              </button>
            );
          })}
