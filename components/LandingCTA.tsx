...
export function PricingCTA({ highlight }: { highlight: boolean }) {
  return (
    <Link
      href="/analyze"
      className={`block text-center py-2.5 rounded-xl text-sm font-medium hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus:ring-4 focus:ring-brand-500/10 transition-all duration-200 ${highlight ? "bg-brand-600 hover:bg-brand-700 text-white shadow-md shadow-brand-500/10 hover:shadow-brand-500/20" : "border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 shadow-sm"}`}
    >
...
export function FooterNav() {
  return (
    <nav className="flex gap-6 flex-wrap" aria-label="Footer navigation">
      ...
      <Link href="/analyze" className="hover:text-gray-700 dark:hover:text-gray-200 transition-colors">Analyze</Link>
