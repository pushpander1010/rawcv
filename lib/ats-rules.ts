import type { ParsedResume, ATSIssue } from "@/types";

export function runRuleChecks(parsed: ParsedResume, raw: string): ATSIssue[] {
  ...
  if (parsed.skills && parsed.skills.length > 0 && allBullets.length > 0) {
    ...
    const skillsInBullets = parsed.skills.filter((s) => bulletText.includes(s.toLowerCase()));
...
export function calculateBaseScore(issues: ATSIssue[]): number {
  let deductions = 0;
