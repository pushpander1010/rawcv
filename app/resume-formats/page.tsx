  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <div className="max-w-5xl mx-auto px-6 pb-20 -mt-4 space-y-10">
        {/* Comparison Table */}
        <section className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-md overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
                  <th className="px-4 py-3 text-xs font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wide">Country</th>
                  <th className="px-4 py-3 text-xs font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wide">Pages</th>
                  <th className="px-4 py-3 text-xs font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wide">Photo</th>
                  <th className="px-4 py-3 text-xs font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wide hidden sm:table-cell">Personal Details</th>
                  <th className="px-4 py-3 text-xs font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wide hidden md:table-cell">Style</th>
                </tr>
              </thead>
              <tbody>
                {FORMATS.map((format) => (
                  <tr key={format.country} className="border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800 dark:bg-slate-800 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span className="text-lg">{format.flag}</span>
                        <span className="text-sm font-bold text-slate-900 dark:text-white">{format.country}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-300">{format.pages}</td>
                    <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-300">{format.photo}</td>
                    <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-300 hidden sm:table-cell">{format.personalDetails}</td>
                    <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-300 hidden md:table-cell">{format.style}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Detailed Cards */}
        <section>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">Detailed Format Guides</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {FORMATS.map((format) => (
              <div key={format.country} className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-md hover:border-brand-300 transition-all">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <span className="text-3xl">{format.flag}</span>
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white">{format.country}</h3>
                  </div>
                  {format.supported && (
                    <span className="px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-200 text-[10px] font-bold uppercase">
                      AI Conversion
                    </span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="rounded-xl bg-slate-50 dark:bg-slate-800 p-3">
                    <p className="text-[10px] font-semibold uppercase text-slate-500 dark:text-slate-300">Pages</p>
                    <p className="text-xs font-bold text-slate-900 dark:text-white">{format.pages}</p>
                  </div>
                  <div className="rounded-xl bg-slate-50 dark:bg-slate-800 p-3">
                    <p className="text-[10px] font-semibold uppercase text-slate-500 dark:text-slate-300">Photo</p>
                    <p className="text-xs font-bold text-slate-900 dark:text-white">{format.photo}</p>
                  </div>
                </div>

                <div className="mb-3">
                  <p className="text-[10px] font-semibold uppercase text-slate-500 dark:text-slate-300 mb-1">Section Order</p>
                  <div className="flex flex-wrap gap-1">
                    {format.sectionOrder.map((section, i) => (
                      <span key={i} className="text-[10px] text-slate-600 dark:text-slate-300">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300 italic mb-4">💡 {format.keyTip}</p>
                ...
                {format.supported ? (
                  ...
                  <Link href="/international" className="inline-block px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-xs font-bold hover:bg-slate-50 dark:hover:bg-slate-800 dark:bg-slate-800 transition-colors">
                    General Conversion →
                  </Link>
                )}
              </div>
